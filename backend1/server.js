...
app.post("/register", async (req, res) => {
    ...
    db.query(checkUser, [userEmail], async (err, result) => {
        ...
        try {
            ...
            db.query(
                sql,
                [name.trim(), userEmail, hashPassword],
                async (err, result) => {
                    if (err) {
...
// =====================================================
// IMPORTANT: Specific routes MUST come BEFORE generic
// parameterized routes to avoid Express matching
// "search" or "low-stock" as a :user_id value.
// =====================================================

// SEARCH MEDICINE — must be defined BEFORE /medicines/:user_id
...
// LOW STOCK MEDICINES — must be defined BEFORE /medicines/:user_id
...
app.get("/customer/:user_id/:customer_id", (req, res) => {
    ...
    const sql = `
        SELECT customer_name, mobile, doctor_name, visit_date
        FROM customers
        WHERE user_id=?
        AND customer_id=?
...
app.post("/customers/generate-bill", async (req, res) => {
    ...
    // ⚠️ IMPORTANT: Must use getConnection() to get a single
    // dedicated connection for the transaction. Using db.promise()
    // (pool) directly causes each query to use a different
    // connection, breaking transactions entirely.
    let connection;
    ...
    try {
        ...
        if (customerResult.length > 0) {
            if (
                customerResult[0].customer_name.toLowerCase() !==
                ...
                return res.json({
                    success: false,
                    message: "Customer Name does not match Customer ID."
                });
            }
        }
        ...
        for (const item of items) {
            ...
            grandTotal += Number(med.price) * requestedQuantity;
        }
        ...
        for (const item of items) {
            ...
            const total =
                Number(med.price) * Number(item.quantity);
            ...
            if (updateResult.affectedRows === 0) {
                ...
                return res.json({
                    success: false,
                    message: `${med.medicine_name} stock update failed`
                });
            }
        }
        ...
        console.error("Generate Bill Error:", err.code, err.message);
        if (connection) {
        ...
        return res.status(500).json({
            success: false,
            message: `Bill generation failed: ${err.message || "Unknown error"}`
        });
    }
});
...
// SEARCH CUSTOMER HISTORY — must be before /customers/history/:user_id
